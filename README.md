...
1º Tabagismo é o fator dominante: Fumantes têm custos médios significativamente superiores aos não-fumantes (diferença estatisticamente significativa com p < 0.001).
2º Idade e BMI têm relação não-linear: As transformações polinomiais melhoraram a performance, indicando que o aumento de custos acelera com idade e obesidade.
3º Segmentação de clientes: Os três clusters identificados representam perfis de risco distintos, úteis para estratégias diferenciadas de precificação.
4º Features de interação são relevantes: A combinação BMI × idade aparece entre as top features, sugerindo efeito sinérgico destes fatores.
